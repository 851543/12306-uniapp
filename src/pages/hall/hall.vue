...
.header {
  ...
  .more {
    width: 150rpx; /* 调整宽度与header-left对应 */
    text-align: right;
    font-weight: bold;
  }
}
...
/* 日期导航栏样式 */
.date-nav {
  position: fixed; /* 固定在顶部，在header下方 */
  top: 92rpx; /* 约等于header的高度 (15px padding * 2 + 16px font-size * 2 = 30 + 32 = 62px, 实际需要根据header实际高度调整) */
  left: 0;
  right: 0;
  z-index: 99; /* 略低于header */
  display: flex;
  justify-content: space-around;
  align-items: center;
  padding: 20rpx; /* 转换为rpx */
  /* 渐变蓝色背景 */
  background-image: linear-gradient(to right, #409eff, #67b0ff);
  color: white;
  font-size: 28rpx; /* 转换为rpx */
}
...
.train-list {
  ...
  overflow-y: auto;
  padding: 20rpx; /* 转换为rpx */
  margin-top: 160rpx; /* 留出顶部固定区域的高度 (header 92rpx + date-nav 60rpx (20padding*2+20font=60)) */
  margin-bottom: 88rpx; /* 留出底部筛选条的高度 (15px padding * 2 + 14px font-size * 2 = 30 + 28 = 58px, 实际需要根据footer实际高度调整) */
}

/* 单个车次项样式 */
.train-item {
  background-color: white;
  margin-bottom: 20rpx; /* 转换为rpx */
  padding: 30rpx; /* 转换为rpx */
  border-radius: 16rpx; /* 转换为rpx */
  box-shadow: 0 4rpx 8rpx rgba(0, 0, 0, 0.1); /* 转换为rpx */
...
.filter-bar {
  ...
  justify-content: space-around;
  align-items: center;
  padding: 30rpx 0; /* 转换为rpx */
  background-color: white;
  border-top: 2rpx solid #eee; /* 转换为rpx */
  .filter-item {
    font-size: 28rpx; /* 转换为rpx */
    color: #333;
  ...
  .filter-item:not(:first-child) {
    padding-left: 40rpx; /* 转换为rpx */
  }
  .filter-item:not(:first-child)::before { /* ADD THIS BLOCK */
    content: '';
    position: absolute;
    left: 0;
    top: 50%;
